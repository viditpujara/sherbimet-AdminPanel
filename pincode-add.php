...
//     $city_id = mysqli_real_escape_string($conn, $_POST['city_id']);
    ...
    $query_check = mysqli_query($conn, "select lower(pincode) from $table where pincode=lower('{$pincode}')") or die(mysqli_error($conn));

  $count = mysqli_num_rows($query_check);
  
  if($count>0)
  {
         echo "<script>alert('$page_title Already Exist');window.location='$add_link';</script>";
  }
else{
    $queryins = mysqli_query($conn, "insert into $table(pincode,insert_datetime)values('{$pincode}','{$datetime}')") or die(mysqli_error($conn));
                  ...
                  <div class="row">
                    <div class="col-sm-3">
                      ...
                      <div class="form-group">
                        <label>Pincode</label>
                        <input type="number" maxlength="6" name="pincode" class="form-control" placeholder="Enter Pincode" required="">
                      </div>
                    </div>
